  ...
  if input.status == "approved" and input.approvedGames then
    for _, game_uri in ipairs(input.approvedGames) do
      local existing = db.raw(
        "SELECT uri FROM records WHERE collection = 'games.gamesgamesgamesgames.claimReview' AND record::jsonb->>'status' = 'approved' AND EXISTS (SELECT 1 FROM jsonb_array_elements_text(record::jsonb->'approvedGames') AS elem WHERE elem = $1) LIMIT 1",
        { game_uri }
      )
