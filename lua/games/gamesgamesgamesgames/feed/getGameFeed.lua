...
local function get_like_count(game_uri)
  local result = db.raw(
    "SELECT COUNT(*) AS count FROM happyview_records WHERE collection = $1 AND record::jsonb->>'subject' = $2",
...
local function get_viewer_like(game_uri)
  ...
  local result = db.raw(
    "SELECT uri FROM happyview_records WHERE collection = $1 AND did = $2 AND record::jsonb->>'subject' = $3 LIMIT 1",
